# from cgitb import grey
# from itertools import count
# from unittest.util import _count_diff_all_purpose
# import cv2
# import numpy as np

# # capture video
# capture = cv2.VideoCapture('video.mp4')

# # assigning min width & height for rectangle
# min_width_rect=80
# min_height_rect=80



# # algo generate
# # algo = cv2.bgsegm.createBackgroundSubtractorMOG()
# # algo = cv2.bgsegm.
# algo = cv2.createBackgroundSubtractorMOG2()


# # center handling function
# def center_handle(x,y,w,h):
#     x1=int(w/2)
#     y1=int(y/2)
#     cx = x+x1
#     cy = y+y1
#     return cx,cy


# # counting list
# detect=[]
# count_line_position=550
# offset = 6
# counter = 0

# while True:
#     ret, frame1 = capture.read()
#     grey = cv2.cvtColor(frame1, cv2.COLOR_BGR2GRAY)
#     blur = cv2.GaussianBlur(grey, (3, 3), 5)
#     # applying on each frame1
#     img_sub = algo.apply(blur)
#     dilate1 = cv2.dilate(img_sub, np.ones((5, 5)))
#     kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
#     dilate2 = cv2.morphologyEx(dilate1, cv2.MORPH_CLOSE, kernel)
#     dilate2 = cv2.morphologyEx(dilate2, cv2.MORPH_CLOSE, kernel)
#     counter1, h = cv2.findContours(dilate2, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    
    
#     # Draw a line to count
#     # cv2.line(frame1,(25,25),(1200,200),(0,0,255),3.3 )
#     # cv2.line(frame1, (25,600),(1260,590),(255,0,0), 3)
#     cv2.line(frame1, (25,count_line_position),(1260,count_line_position),(255,0,0), 3)
    
#     # draw a rectangle
#     for(i,c) in enumerate(counter1):
#         (x,y,w,h) = cv2.boundingRect(c)
#         validate_counter = (w >= min_width_rect) and (h>= min_height_rect)
#         if not validate_counter:
#             continue
        
#         cv2.rectangle(frame1, (x,y),(x+w,y+h),(0,255,0),3)
#         center=center_handle(x,y,w,h)
#         detect.append(center)
#         cv2.circle(frame1,center,4,(0,0,255),-1)
        
#         for (x,y) in detect:
#             if y<(count_line_position+offset) and y>(count_line_position-offset):
#                 counter+=1
#             cv2.line(frame1, (25,count_line_position),(1200,count_line_position),(255,300,160), 3)    
#             detect.remove((x,y))
#             print(f"Vehicle Count = {counter}") 
            
#     cv2.putText(frame1,"VEHICLE CLOUNT : "+str(counter),(450,70),cv2.FONT_HERSHEY_SIMPLEX,2,(255,0,50),5)        
#     # cv2.imshow("detect", dilate2)

#     cv2.imshow("video banaye", frame1)

#     if cv2.waitKey(1) & 0xFF == ord('q'):
#         break

# cv2.destroyAllWindows()
# capture.release()

# --------------------------------------------------------------------------------------------------------------------
import cv2
import numpy as np

#Web Camera
cap = cv2.VideoCapture('video.mp4')

min_width_rectangle = 80
min_height_rectangle = 80

count_line_position = 550
# Initialize Substructor
algo = cv2.createBackgroundSubtractorMOG2(history=100, varThreshold=40)

def center_handle(x,y,w,h):
    x1=int(w/2)
    y1=int(h/2)
    cx=x+x1
    cy=y+y1
    return cx,cy

detect = []
offset = 6 #Alowable error b/w pixel
counter = 0

while True:
    ret, video = cap.read()
    gray = cv2.cvtColor(video, cv2.COLOR_BGR2GRAY)
    blur = cv2.GaussianBlur(gray, (3,3), 5)
    
# Applying on each frame
    vid_sub = algo.apply(blur)
    dilat = cv2.dilate(vid_sub, np.ones((5,5)))
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5))
    dilatada = cv2.morphologyEx(dilat, cv2.MORPH_CLOSE, kernel)
    dilatada = cv2.morphologyEx(dilatada, cv2.MORPH_CLOSE, kernel)
    countersahpe, h = cv2.findContours(dilatada, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    cv2.line(video, (25,count_line_position),(1200,count_line_position),(255,0,0), 3)

    for (i, c) in enumerate(countersahpe):
        (x,y,w,h) = cv2.boundingRect(c)
        val_counter = (w>=min_width_rectangle) and (h>= min_height_rectangle)
        if not val_counter:
            continue
        cv2.rectangle(video,(x,y),(x+w,y+h),(0,255,255),2)
        cv2.putText(video,"Vehicle No: " + str(counter), (x,y-20),cv2.FONT_HERSHEY_TRIPLEX,1,(255,244,0),2)


        center = center_handle(x,y,w,h)
        detect.append(center)
        cv2.circle(video, center, 4, (0,0,255), -1)

        for (x,y) in detect:
            if y<(count_line_position + offset) and  y>(count_line_position - offset):
                counter+=1
                cv2.line(video, (25,count_line_position),(1200,count_line_position),(0,127,255), 3)
                detect.remove((x,y))

                print("Vehicle No: "+ str(counter))

    cv2.putText(video,"Vehicle No: " + str(counter), (450,70),cv2.FONT_HERSHEY_SIMPLEX,2,(0,0,255),5)

    cv2.imshow('Detector',video)


    if cv2.waitKey(1) == ord('q'):
        break

cap.release()
cv2.destroyAllWindows()